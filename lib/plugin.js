...
var Plugin = function(name, proto) {
    ...
    if (!Array.isArray(this.deployerDirectories)) {
        this.deployerDirectories = [];
        ...
        if (fs.existsSync(deployerBaseDir)) {
            fs.readdirSync(deployerBaseDir).forEach(function(f) {
                this.deployerDirectories.push(path.join(deployerBaseDir, f));
            }.bind(this));
        }
    }
};
...
plugin.getDeployerDirectories = function() {
    'use strict';
    ...
    Object.keys(plugins).forEach(function(i) {
        plugins[i].deployerDirectories.forEach(function(dir) {
            result.push(dir);
        });
    });
