...
var reporter = {
    print: function(type, message) {
        ...
        var prefix = '%s';
        var prefixData = config.debug ? sprintf('%08.3f i| ', deltaT()) : 'pas: ';
        ...
        if (typeof message === 'object') {
            ...
            message.trim().split(/\n/).forEach(function(line) {
                var currentLine = line.replace(/[\s]+$/, '');
                if (currentLine === lastLine) {
                    return;
                }
                lastLine = currentLine;
                process.stdout.write(sprintf(prefix + '%s\n', prefixData, line.replace(/[\s]+$/, '')));
            });
        } else {
    ...
    error: function() {
        ...
        var prefix = '%s';
        var prefixData = config.debug ? sprintf('%08.3f e| ', deltaT()) : 'pas: ';
        ...
        if (config.debug && e) {
            process.stderr.write(sprintf(prefix + '<E> %s\n', prefixData, e));
            if (e.stack) {
                e.stack.trim().split(/\n/).forEach(function(line) {
