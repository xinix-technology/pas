...
var populatePristineProviders = function() {
    ...
    if (global.APP_INCLUDES) {
        providers = APP_INCLUDES.providers;
    } else {
        var providerDir = path.join(APP_LIB_DIR, 'providers');
        fs.readdirSync(providerDir).forEach(function(f) {
            providers.push(providerDir + '/' + f);
        });
    }

    // console.log('ppp',providers);
